...
void get_name(Person &person)   //have to give the location in memory (ik from C)
{
    //get user name and save it to the person
    std::cout << "Enter Name: " << std::endl;   //prompt the User
    std::cin >> person.name;    //get the users name from user and then save it in person.name
}
